              ...
              <Card key={balita.id} className="flex flex-col border border-gray-100 shadow-sm rounded-xl bg-white overflow-hidden">
                ...
                <div className="px-5 py-4 flex items-center justify-between">
                  <div className={`px-4 py-1.5 rounded-full text-xs font-bold ${
                    balita.pengukuran?.some(p => p.bulan === currentMonth && p.tahun === currentYear)
                      ? "bg-[#e6fbf5] text-[#0d9488]" 
                      : "bg-[#fff5ea] text-orange-600"
                  }`}>
                    {balita.pengukuran?.some(p => p.bulan === currentMonth && p.tahun === currentYear) ? "Sudah diukur" : "Belum diukur"}
                  </div>
